  ...
  public void fillMenuItems (int id, HeaderView header, LinearLayout menu) {
    if (id == R.id.menu_more) {
      if (mediaLayout.getMode() != MediaLayout.MODE_AVATAR_PICKER) {
        ...
        // Botao "Selecionar Tudo" aparece apenas fora do modo avatar
        header.addButton(menu, R.id.btn_selectAll, R.drawable.baseline_done_all_24, 49f, this, getHeaderIconColorId());
      }
    ...
    if (id == R.id.menu_btn_clear) {
      ...
      if (adapter != null) {
        ...
        if (adapter.areAllSelected()) {
          adapter.deselectAll(lm);
          mediaLayout.clearCounter();
        } else {
          adapter.selectAll(lm);
