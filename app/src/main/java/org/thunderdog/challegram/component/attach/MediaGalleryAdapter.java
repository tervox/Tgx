...
public class MediaGalleryAdapter extends RecyclerView.Adapter<MediaGalleryAdapter.MediaHolder> implements View.OnClickListener, View.OnLongClickListener, MeasuredAdapterDelegate, MediaGalleryImageView.ClickListener {
  ...
  public boolean areAllSelected () {
    return images != null && !images.isEmpty() && selected.size() == images.size();
  }

  /**
   * Seleciona todos os itens do bucket atual de uma vez.
   * Notifica o callback com o total selecionado.
   */
  public void selectAll (GridLayoutManager layoutManager) {
    if (images == null || images.isEmpty()) return;
    selected.clear();
    selected.addAll(images);
    notifyDataSetChanged();
    if (callback != null && !images.isEmpty()) {
      callback.onPhotoOrVideoSelected(selected.size(), images.get(images.size() - 1), selected.size() - 1);
    }
  }
   ...
   */
  public void deselectAll (GridLayoutManager layoutManager) {
    if (selected.isEmpty()) return;
    // Guarda referencia de um item para o callback antes de limpar
    ImageFile lastItem = images != null && !images.isEmpty() ? images.get(0) : null;
    selected.clear();
    notifyDataSetChanged();
    if (callback != null && lastItem != null) {
      callback.onPhotoOrVideoSelected(0, lastItem, -1);
    }
  }
